"""Actual J1/A2/C2/P2 -> S2 allocation; native debit and custody stay unchanged."""
...
class ObservedSecondReserveTests(unittest.TestCase):
    def setUp(self):
        ...
        if self._testMethodName == "test_S2_requires_its_source_path_in_original_I_inventory":
            producer = accounting.register_simulated_resource_initial_inventories
            def without_S_path(registry):
                path = registry.policy.root / "src/scientist_one/simulated_reserve.py"
                path.rename(registry.policy.root / "retained-uninventoried-S-source.py")
                return producer(registry)
            with mock.patch.object(accounting, "register_simulated_resource_initial_inventories", without_S_path):
                self.a.setUp()
        else:
            self.a.setUp()
    ...
    def test_live_configuration_source_external_and_custody_drift_refuse_new_S2(self):
        paths = [self.root / "configs/resource_limits.json", self.root / "src/scientist_one/simulated_reserve.py",
                 self.root / observation._journal_path(self.case.charge)]
        tail = sorted((self.root / ".scientist-one-build/resource-authority" / self.case.case.run).glob("*.json"))[-1]
        paths.append(tail)
        for path in paths:
    ...
    def test_S2_requires_its_source_path_in_original_I_inventory(self):
        self.assertFalse((self.root / "src/scientist_one/simulated_reserve.py").exists())
        self.assertTrue((self.root / "retained-uninventoried-S-source.py").is_file())
        self.assert_refuses_unchanged(self.publish)

    def test_static_readback_retains_native_guards_and_late_source_refuses_return(self):
        before = self.snapshot()
        original = reserve._require_observed_second_at_snapshot
        calls = []
        def drift(*args, **kwargs):
            ...
            calls.append(True)
            path = self.root / "src/scientist_one/simulated_reserve.py"
            path.write_bytes(path.read_bytes() + b"# actual late static-readback source drift\n")
            return result
        with mock.patch.object(reserve, "_require_observed_second_at_snapshot", drift):
            with self.assertRaises(reserve.SimulatedReserveError):
                self.publish()
...
class ObservedSecondReserveMarkerTests(unittest.TestCase):
    def test_v3_metadata_only_markers_refuse_before_legacy_I_or_S_publication(self):
        for schema in ("sim-reserve/v3", "sim-reserve-event/v3"):
            for kind, fixture_type in (("allocation-only-S1", allocation_fixtures.SimulatedReserveTests),
