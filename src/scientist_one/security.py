...
def validate_command(
    ...
    *,
    ...
    if basename.lower().startswith(("python", "pypy")):
        # Python's option grammar accepts attached and combined spellings such
        # as ``-cCODE``, ``-mMOD`` and ``-I-cCODE``.  Searching for exact
        # tokens is therefore not an execution boundary.  The supported local
        # experiment form is deliberately one closed grammar: the three
        # isolation flags in their canonical order, one confined source
        # script, then ordinary script arguments.  Module, inline, stdin,
        # option-terminator and alternate interpreter-option forms are never
        # admitted, even when an obsolete caller supplies a module allowlist.
        del allowed_python_modules
        if len(argv) < 5 or argv[1:4] != ("-I", "-S", "-B"):
            raise CommandSecurityError(
                "Python requires the exact isolated -I -S -B script form"
            )
        script = argv[4]
        if (
            root is None
            ...
            raise CommandSecurityError("Python must run one confined source script")
        for argument in argv[1:]:
            if (
                argument == "-"
                or argument == "--"
                ...
                raise CommandSecurityError(
                    "inline, module, stdin, or ambiguous Python execution is forbidden"
                )
        resolve_confined(
            root,
            script,
