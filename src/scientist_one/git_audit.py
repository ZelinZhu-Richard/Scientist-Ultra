...
GIT_AUDIT_PROFILE = "COMPLETE_SHA1_GIT_INDEX_V2"
...
_LIMITATIONS = (
    ...
    "PACK_WRITE_REVERSE_INDEX_FALSE_REQUIRED_NO_REV_OR_OTHER_ACCELERATORS",
...
class _ObjectStream:
    ...
    def feed(self, chunk: bytes) -> None:
        ...
        while True:
            ...
            self.header = None
            _scan(payload, ".git/decoded-objects/" + oid, binary=kind == "tree")
...
class GitAuditSession:
    ...
    def observe(self, path: str, payload: bytes) -> None:
        ...
        try:
            ...
            _scan(payload, path, binary=kind in {"object", "pack", "index"})
            relative = path[5:]
            if relative == "config":
                _config(payload)
            elif relative == "index":
                ...
                self._put(relative, payload)
            elif kind in {"object", "pack"} or relative == "packed-refs" or relative.startswith("logs/"):
                self._put(relative, payload)
            self._seen.add(path)
        except _Refusal as exc:
            self._failure = exc
    ...
    def finalize(self) -> GitAuditCoverage:
        ...
        try:
            ...
            headers.finish()
            result = self._run(("fsck", "--full", "--strict", "--no-dangling", "--no-progress"), output_limit=MAX_DIAGNOSTIC_BYTES)
            if result:
                raise _Refusal("unexpected_git_validation_output")
            objects = _ObjectStream(headers.objects)
            self._run(("cat-file", "--batch-all-objects", "--batch"), output_limit=MAX_DECODED_BYTES + MAX_OBJECTS * 162, consume=objects.feed)
            objects.finish()
        ...
        return GitAuditCoverage(
            ...
            tuple(sorted(path for path, kind in self._classes.items() if kind in {"object", "pack", "index"})) if failure is None else (),
            (GitAuditFinding(failure.code, failure.path),) if failure is not None else (),
        )


__all__ = ["GitAuditCoverage", "GitAuditError", "GitAuditFile", "GitAuditFinding", "GitAuditSession"]
