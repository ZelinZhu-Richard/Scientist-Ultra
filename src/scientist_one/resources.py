...
class ResourceController:
    ...
    def export_state(self, now: float | None = None) -> ResourceRuntimeState:
        # Persist the last already-observed monotonic instant by default.  This
        # makes snapshot/checkpoint serialization side-effect-free for injected
        # clocks and cannot reset or manufacture elapsed budget on restart.
        with self._lock:
            ...
            elapsed = max(elapsed, wall_observed - self._wall_started_at_epoch_seconds)
            validity = self.validity_budget.snapshot() if self.validity_budget else None
            crashes = dict(sorted(self._crashes.items()))
            checkpoint_elapsed = max(0.0, self._last_checkpoint_at - self._started_at)
            progress_elapsed = max(0.0, self._last_progress_at - self._started_at)
        return ResourceRuntimeState(
            ...
            wall_started_at_epoch_seconds=wall_observed - elapsed,
    ...
    def checkpoint_due(self, now: float | None = None) -> bool:
        with self._lock:
            ...
            return current - self._last_checkpoint_at >= self.config.checkpoint_interval_seconds
    ...
    def snapshot(self, now: float | None = None) -> ResourceSnapshot:
        ...
        return ResourceSnapshot(
            ...
            worker_crashes=crashes,
            stalled_seconds=max(0.0, current - self._last_progress_at),
    ...
    def evaluate(
        ...
        *,
        ...
        if sample.wall_elapsed_seconds >= self.config.maximum_wall_clock_seconds:
            stop_reasons.append("WALL_CLOCK_BUDGET_EXHAUSTED")
        if sample.artifact_bytes + estimate >= self.config.maximum_artifact_bytes:
        ...
        if sample.disk_total_bytes is None or sample.disk_free_bytes is None:
            ...
            reserve = conservative_disk_reserve(
                ...
                self.config.minimum_free_disk_bytes,
                self.config.minimum_free_disk_fraction,
            )
        ...
        elif memory_fraction >= self.config.memory_hard_fraction:
            pause_reasons.append("MEMORY_HARD_LIMIT_REACHED")
        elif memory_fraction >= self.config.memory_soft_fraction:
        ...
        if sample.worker_crashes >= self.config.worker_crash_limit:
            pause_reasons.append("REPEATED_WORKER_CRASHES")
        if sample.stalled_seconds >= self.config.stall_timeout_seconds:
            pause_reasons.append("WORK_STALLED")
        if (
            sample.artifact_growth_bytes_per_second
            >= self.config.maximum_artifact_growth_bytes_per_second
        ):
            pause_reasons.append("ABNORMAL_ARTIFACT_GROWTH")

        if (
            sample.concurrent_experiments + requested_experiments
            > self.config.maximum_concurrent_experiments
        ):
            pause_reasons.append("CONCURRENCY_LIMIT_REACHED")
        if sample.cpu_workers + requested_cpu_workers > self.effective_cpu_worker_limit:
            pause_reasons.append("CPU_WORKER_LIMIT_REACHED")
        if sample.gpu_jobs + requested_gpu_jobs > self.config.gpu_job_limit:
            pause_reasons.append("GPU_JOB_LIMIT_REACHED")

        if validity_units:
            if self.validity_budget is None or validity_stage is None:
                pause_reasons.append("VALIDITY_BUDGET_NOT_CONFIGURED")
            elif not self.validity_budget.can_charge(validity_stage, validity_units):
                stop_reasons.append(
        ...
        checkpoint_elapsed = self._last_checkpoint_at - self._started_at
        if (
        ...
        return ResourceDecision(
            ...
            backoff_seconds=(self.backoff_seconds(max(1, sample.worker_crashes)) if not accept_new_work else 0.0),
