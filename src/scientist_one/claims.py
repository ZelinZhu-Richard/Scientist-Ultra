...
# Resolver identity is part of every verification-receipt digest.  Claim graph
# producers and independent state validators therefore use one versioned,
# provider-independent identity so that a fresh re-resolution can reproduce
# the exact receipt bytes rather than merely an equivalent boolean outcome.
CLAIM_GRAPH_RESOLVER_ID = "artifact-registry-claim-evidence-v1"
...
class ClaimEvidenceUse(StrEnum):
    """How an eligible claim may be used downstream.

    Eligibility establishes graph completeness and consistency.  This
    independent classification establishes whether the resulting claim may be
    used as scientific paper evidence, retained as a system fixture, or kept
    only for non-evidentiary diagnostics.
    """
...
class EvidenceVerificationReceipt:
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "EvidenceVerificationReceipt":
        expected = {
            "evidence_id",
            "evidence_kind",
            "artifact_hash",
        ...
        if not isinstance(value, Mapping) or set(value) != expected:
            raise ClaimValidationError(
                "evidence verification receipt schema is invalid"
            )
        try:
            return cls(
                ...
                registry_record_hash=value["registry_record_hash"],
                support_receipt_hash=value["support_receipt_hash"],
                support_receipt_record_hash=value["support_receipt_record_hash"],
                support_verifier_id=value["support_verifier_id"],
                support_verifier_role=Role(value["support_verifier_role"]),
            ...
            raise ClaimValidationError(
                "evidence verification receipt is malformed"
            ) from exc
...
class MaterialClaim:
    ...
    # Omission is safe for legacy callers; scientific use requires an explicit choice.
        ...
        if not isinstance(self.evidence_use, ClaimEvidenceUse):
            try:
                object.__setattr__(
                    self,
                    "evidence_use",
                    ClaimEvidenceUse(self.evidence_use),
                )
            except (TypeError, ValueError) as exc:
                raise ClaimValidationError(
                    "evidence_use must be a ClaimEvidenceUse"
                ) from exc
        ...
        if not isinstance(self.claim_evidence_use, ClaimEvidenceUse):
            try:
                object.__setattr__(
                    self,
                    "claim_evidence_use",
                    ClaimEvidenceUse(self.claim_evidence_use),
                )
            except (TypeError, ValueError) as exc:
                raise ClaimValidationError(
                    "support receipt claim evidence use is invalid"
                ) from exc
    ...
    *,
    ...
    def resolve(
        ...
        if (
            ...
            or support_receipt.claim_evidence_use is not claim.evidence_use
...
class ClaimEvidenceGraph:
        ...
        *,
        ...
        if claim.confirmatory and not confirmatory_evidence_valid:
            raise ClaimNotEligibleError(
                f"claim {claim_id!r} lacks freshly revalidated confirmatory authority"
            )
    ...
    def writer_view(
        self,
        *,
        include_nonscientific: bool = False,
        confirmatory_evidence_valid: bool = False,
    ) -> tuple[dict[str, Any], ...]:
        """Return a diagnostic projection of structurally eligible graph claims.

        This view is never paper or promotion authority. Production consumers
        must freshly resolve canonical research state plus the claim-specific
        semantic, qualifier, reference-support, and custody authorities. The
        default merely filters out system fixtures and non-evidentiary claims;
        ``include_nonscientific=True`` exposes them for diagnostics.
        """

        if not isinstance(include_nonscientific, bool):
            raise ClaimValidationError("include_nonscientific must be boolean")
        if not isinstance(confirmatory_evidence_valid, bool):
            raise ClaimValidationError("confirmatory_evidence_valid must be boolean")
        ...
        for claim in self.claims:
            ...
            if include_nonscientific:
                view["evidence_use"] = claim.evidence_use.value
            result.append(view)
        return tuple(result)
            ...
            claims.append(
                {
                    ...
                    "confirmatory": claim.confirmatory,
                    "evidence_use": claim.evidence_use.value,
                    "evidence_links": [
        ...
        try:
            ...
            for item in value["claims"]:
                ...
                claim = MaterialClaim(
                    ...
                    evidence_use=ClaimEvidenceUse(item["evidence_use"]),
                )
...
def writer_view(
    ...
    *,
    ...
    """Return the graph's diagnostic-only, non-authoritative writer projection."""
    ...
    return graph.writer_view(
        include_nonscientific=include_nonscientific,
