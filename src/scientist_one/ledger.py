...
from .errors import LedgerCorruptionError, LedgerError, ValidationError
from .models import (
    State,
    evaluation_to_dict,
    freeze_json,
...
STATE_CHANGING_EVENT_TYPES = frozenset({"TRANSITION", "SECURITY_STOP"})
...
class LedgerEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.state_before != self.requested_state_after
            and self.event_type not in STATE_CHANGING_EVENT_TYPES
        ):
            raise ValidationError(
                "only transition and security-stop events may change macro state"
            )
        if (
            self.event_type == "SECURITY_STOP"
            and self.requested_state_after.value != "STOP_SECURITY"
        ):
            raise ValidationError("security-stop events must enter STOP_SECURITY")
...
class EventLedger:
    ...
    def _validate_bytes(data: bytes) -> LedgerValidationResult:
        ...
        for line_number, raw_line in enumerate(data.splitlines(keepends=True), 1):
            ...
            if events and event.state_before != events[-1].requested_state_after:
                return LedgerValidationResult(
                    False,
                    tuple(events),
                    expected_prior,
    ...
    def _append_locked(
        ...
        try:
            ...
            validated_prefix = self._read_ledger_fd(descriptor)
            result = self._validate_bytes(validated_prefix)
            if not result.valid:
                raise LedgerCorruptionError(
                    f"ledger validation failed at line {result.error_line}: {result.error}"
                )
            ...
            if (
                result.events
                and event.state_before
                != result.events[-1].requested_state_after
            ):
                raise LedgerError("event state does not match current ledger head")
            ...
            if self._read_ledger_fd(descriptor) != validated_prefix:
                raise LedgerError(
                    "ledger bytes changed outside the serialized append lock"
                )
            written = 0
            ...
            committed_bytes = self._read_ledger_fd(descriptor)
            if committed_bytes != validated_prefix + line:
                raise LedgerError("ledger append bytes are inconsistent")
            committed = self._validate_bytes(committed_bytes)
            if (
                not committed.valid
                or committed.event_count != result.event_count + 1
                or committed.head_hash != event.event_hash
                or committed.events[-1] != event
            ):
                raise LedgerCorruptionError(
                    "ledger append did not commit the exact validated event"
                )
            return event
        except OSError as exc:
