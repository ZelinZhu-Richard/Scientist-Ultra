...
from .reproduction import ReproductionError, verify_frozen_reproduction
...
from .state_machine import macro_transition_contracts
...
EVALUATION_RECEIPT_KEYS = {
    "evaluator_class", "authority", "decision", "critical_objection",
    "artifact_hashes", "evaluation_sha256", "frozen_context_sha256",
    "producer_role", "reason", "r_checks", "logically_separated",
...
def _validate_resource_authority_projection(
    ...
    for event in events:
        ...
        if positions:
            ...
            if position >= len(event.artifact_hashes) or len(observed) >= len(expected):
        ...
        for key, raw_output in zip(keys, outputs, strict=True):
            ...
            try:
                evaluation = Evaluation(
                    EvaluatorClass(output["evaluator_class"]),
                    Role(output["authority"]),
                    Decision(output["decision"]),
                    tuple(output["artifact_hashes"]),
                    output["reason"],
                    tuple(RCheck(item) for item in output.get("r_checks", ())),
                    bool(output.get("critical_objection", False)),
                    Role(output["producer_role"])
                    if output.get("producer_role")
                    else None,
                )
            except (KeyError, TypeError, ValueError) as exc:
                raise PackagingError("ledger evaluator receipt is malformed") from exc
            if evaluation.sha256 != output.get("evaluation_sha256"):
...
def _verify_finalized_package(
    ...
    for logical_type, entry in artifact_index.items():
        ...
        if (
            ...
            or archived_metadata.get("logical_type") != logical_type
        ):
    ...
    for logical_type, entry in artifact_index.items():
        ...
        if (
            ...
            or entry.get("logical_type") != logical_type
    ...
    if (
        ...
        or set(raw_final_receipt) != {
            "run_id", "prior_state", "current_state", "idempotency_key",
            "request_fingerprint", "replayed", "generated_artifact_types",
        }
    ...
    if set(artifact_projection) != {
        record.logical_type for record in records_by_hash.values()
    }:
        raise PackagingError("final manifest logical artifact names differ from registry")
    for logical_type, projected in artifact_projection.items():
        ...
        if dict(projected) != expected_projection or logical_type != stored.logical_type:
...
def _validate_custody_snapshot(
    ...
    if (
        ...
        or record.get("confirmatory_claims_valid") is not True
        ...
        or not status.confirmatory_claims_valid
...
def package_run(root: str | Path, run_id: str) -> PackageResult:
    ...
    if not isinstance(r_checks, dict) or any(
        r_checks.get(f"R{index}") != "PASS" for index in range(8)
    ):
        raise PackagingError("all R0-R7 checks must pass")
    if not isinstance(reproduction, dict) or reproduction.get("status") != "PASS":
        raise PackagingError("successful primary reproduction is required")
    ...
    for logical_type, projected in artifact_records.items():
        ...
        if (
            metadata_snapshot.logical_type != logical_type
            or dict(projected)
            != _manifest_projection_from_record(projected, metadata_snapshot)
        ):
            raise PackagingError("declared artifact differs from registry metadata")
    ...
    if not isinstance(frozen_checks, dict) or any(
        frozen_checks.get(f"R{index}", {}).get("status") != "PASS"
        or frozen_checks.get(f"R{index}", {}).get("evidence_derived") is not True
        for index in range(8)
    ):
        raise PackagingError("frozen audit report does not substantiate R0-R7")
    ...
    transition_evaluations = {
        f"{value.get('evaluator_class')}:AUDIT": dict(value)
    ...
    for key in ("E0:AUDIT", "E2:AUDIT", "E3:AUDIT"):
        ...
        for field in (
            ...
            if ledger_stored.get(field) != stored.get(field):
                raise PackagingError(f"ledger evaluator receipt differs: {key}")
        if tuple(ledger_stored.get("artifact_hashes", ())) != tuple(stored.get("artifact_hashes", ())):
            raise PackagingError(f"ledger evaluator artifacts differ: {key}")
        if tuple(ledger_stored.get("r_checks", ())) != tuple(stored.get("r_checks", ())):
            raise PackagingError(f"ledger evaluator R checks differ: {key}")
        try:
            evaluation = Evaluation(
    ...
    if (
        frozen_reproduction_manifest.get("kind") != "FROZEN_REPRODUCTION_MANIFEST"
        or reproduction_result.get("kind") != "REPRODUCTION_RESULT"
        ...
        or reproduction_result.get("status") != "PASS"
    ...
    release_manifest = planned_release_manifest
    if (
        artifact_index != planned_artifact_index
